...
function WithMenu({ text, items = [] }: props) {
  ...
  return (
    ...
    >
      ...
      <div
        className={` ${
          i18n.dir(i18n.language) === "ltr" ? "left-0" : "right-0"
        } absolute bottom-0 w-full h-0.5 bg-transparent group-hover:bg-slate-300/80
            transition-colors duration-500 group-focus-within:bg-slate-300/80`}
      ></div>
      ...
      {items.length > 0 ? (
        <div
          ...
          className={`${
            i18n.dir(i18n.language) === "ltr" ? "left-0" : "right-0"
          } w-48 z-10 absolute bottom-0  translate-y-[95%] shadow-lg
